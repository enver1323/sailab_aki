...
export const PatientLevelData: React.FC<{ value: number; threshold: number }> = ({
  value,
  threshold,
}) => {
  const predLabel = value > threshold ? "AKI" : "정상";
  return (
    <DataRow>
      ...
      <DataCol>
        ...
        <SectionSubTitle>모델 예측: { predLabel }</SectionSubTitle>
        <SectionSubTitle>위험도: {(value * 100).toFixed(2)}%</SectionSubTitle>

        <SectionTitle>실제 결과</SectionTitle>
        <SectionSubTitle>실제 AKI 발생: ...</SectionSubTitle>
      </DataCol>
      <DataCol>
        <SectionTitle>추가 설명</SectionTitle>
        <ul>
          <li>위험도 점수가 {(threshold * 100).toFixed()}% 이상일 때 AKI 발생으로 예측합니다.</li>
