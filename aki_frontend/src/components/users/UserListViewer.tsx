...
import { useUserListData } from "@/hooks/queries/useUserData";
...
const ViewDetailButtonComponent: React.FC<{ id: number }> = ({ id }) => {
  ...
  return <ViewDetailButton onClick={() => navigate(`/admin/users/${id}`)}>Edit</ViewDetailButton>;
};

const columnHelper = createColumnHelper<UserProps>();

const columns: ColumnDef<UserProps, any>[] = [
...
const StyledTable = styled(Table<UserProps>)`
  & tr {
...
const UserListViewer: React.FC = () => {
  const [searchFilter, setSearchFilter] = React.useState<string>("");
  const [page, setPage] = React.useState<number>(1);

  const { data, isLoading, isError } = useUserListData(page, searchFilter);
            ...
            <UserTableWrapper>
              <StyledTable
                tableOptions={{
                  data: data?.items ?? [],
                  columns,
                  getCoreRowModel: getCoreRowModel<UserProps>(),
                  getPaginationRowModel: getPaginationRowModel<UserProps>(),
                  getSortedRowModel: getSortedRowModel<UserProps>(),
                }}
              />
            </UserTableWrapper>
            <Pagination
